...
export default function CreateOrEditRecipePage() {
  ...
  useEffect(() => {
    ...
    axios.get(`/recipe/${slug}`)
      ...
      .catch((err) => {
        alert("Gagal mengambil data resep: " + (err.response?.data?.message || err.message));
        navigate("/recipes");
      });
  }, [slug]);
  ...
  const addField = (updater: React.Dispatch<React.SetStateAction<string[]>>) => {
    updater((prev) => [...prev, ""]);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 to-white p-8">
      <div className="max-w-3xl mx-auto bg-white rounded-3xl shadow-lg p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Judul */}
          <div>
            <label className="block font-medium text-gray-700 mb-1">Judul Resep</label>
            <input
              type="text"
              required
              value={title}
          ...
          {/* Ringkasan */}
          <div>
            <label className="block font-medium text-gray-700 mb-1">Ringkasan</label>
            <textarea
              value={summary}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block font-medium text-gray-700 mb-1">Durasi Memasak (menit)</label>
              <input
                type="number"
                value={duration}
            ...
            <div>
              <label className="block font-medium text-gray-700 mb-1">Tingkat Kesulitan</label>
              <select
                value={difficulty}
          ...
          {/* Bahan */}
          <div>
            <label className="block font-medium text-gray-700 mb-2">Bahan-bahan</label>
            {ingredients.map((item, index) => (
              <input
                key={index}
                value={item}
                onChange={(e) => updateArrayField(setIngredients, index, e.target.value)}
          ...
          {/* Langkah */}
          <div>
            <label className="block font-medium text-gray-700 mb-2">Langkah-langkah</label>
            {steps.map((item, index) => (
              <textarea
                key={index}
                value={item}
                onChange={(e) => updateArrayField(setSteps, index, e.target.value)}
                className="w-full p-3 mb-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500"
          ...
          {/* Tag */}
          <div>
            <label className="block font-medium text-gray-700 mb-2">Tag</label>
            {tags.map((tag, index) => (
              <input
                key={index}
                value={tag}
                onChange={(e) => updateArrayField(setTags, index, e.target.value)}
                className="w-full p-3 mb-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500"
          ...
          <div className="text-right">
            ...
            >
              {loading ? "Menyimpan..." : isEditMode ? "Simpan Perubahan" : "Simpan Resep"}
            </button>
          </div>
        </form>
      </div>
